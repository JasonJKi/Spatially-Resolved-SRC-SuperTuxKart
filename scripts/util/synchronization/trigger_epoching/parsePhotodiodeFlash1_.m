function [flash_index, timeseries_norm] = parsePhotodiodeFlash1_(photodiode, event_code, is_debug, quick_fix)
disp('Epoching Flash Event From the timeseriesriggers. on and off event')
...
min_val = min(unique(photodiode.timeseries));
max_val = max(unique(photodiode.timeseries));
timeseries_norm = (1-0)/(max_val-min_val)*(photodiode.timeseries-max_val)+1;

% get ppt flash and frame marker events
% index frame marking events based on the trigger identifier
flash_index = find(timeseries==event_code);
...
flash_index = filter3(flash_index, timestamp);
...
[flash_index] =  filter5(flash_index, timestamp, timeseries_norm);
...
if is_debug
    figure;clf;hold on;
    time = timestamp;
    plot(time, timeseries_norm); % plot all events
    plot(time(flash_index), timeseries_norm(flash_index),'.r');
    plot(time([flash_index(1) flash_index(end)]), timeseries_norm([flash_index(1) flash_index(end)]),'*g');
...
function flash_index = filter3(flash_index, timestamp)
...
function [flash_index] =  filter5(flash_index, timestamp, timeseries)
% check that last on event is not a run on and that it ends at a given
% moment.

    timestamp = timestamp;
    timestamp(timeseries==0) = 0;
    flash_boolean = diff([0; timestamp])>0.1;
%     flash_event_index = find(flash_boolean);
    timestamp(find(flash_boolean~=0)+1) = 0;
%     clf
%     subplot(2,1,1); plot(timeseries)
%     subplot(2,1,2); hold on; plot(timestamp, timeseries); plot(timestamp(flash_index),flash_boolean(flash_index),'.r')
...
%     plot(timestamp(end_index), ones(length(current_end_flash), 1),'og')
%     plot(timestamp(possible_end_index), ones(length(possible_end_flash), 1),'*k')
